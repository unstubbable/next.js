...
describe('distDir', () => {
    ...
    ;(process.env.TURBOPACK_DEV ? describe.skip : describe)(
      ...
      () => {
        ...
        it('should build the app within the given `dist` directory', async () => {
          expect(
            await fs.pathExists(join(__dirname, `/../dist/${BUILD_ID_FILE}`))
          ).toBeTruthy()
        })
        it('should not build the app within the default `.next` directory', async () => {
          expect(await fs.pathExists(join(__dirname, '/../.next'))).toBeFalsy()
        })
      }
    )
  })
  ;(process.env.TURBOPACK_BUILD ? describe.skip : describe)(
    ...
    () => {
      ...
      it('should build the app within the given `dist` directory', async () => {
        expect(
          await fs.pathExists(join(__dirname, `/../dist/${BUILD_MANIFEST}`))
        ).toBeTruthy()
      })
      it('should not build the app within the default `.next` directory', async () => {
        expect(await fs.pathExists(join(__dirname, '/../.next'))).toBeFalsy()
