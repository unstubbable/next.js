...
function runTests(isDev) {
  if (!isDev) {
    it('should output prerendered index routes correctly', async () => {
      expect(await fs.pathExists(join(buildPagesDir, 'pages/en-US.html'))).toBe(
        true
      )
      expect(await fs.pathExists(join(buildPagesDir, 'pages/en-US.json'))).toBe(
        true
      )
      expect(await fs.pathExists(join(buildPagesDir, 'pages/fr.html'))).toBe(
        true
      )
      expect(await fs.pathExists(join(buildPagesDir, 'pages/fr.json'))).toBe(
        true
      )
    })
  }
