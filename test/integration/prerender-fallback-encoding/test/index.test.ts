    ...
    it('should output paths correctly', async () => {
      for (const path of prerenderedPaths) {
        for (const mode of modePaths) {
          ...
          expect(
            await fs.pathExists(join(pagesDir, mode, path + '.html'))
          ).toBe(true)
          expect(
            await fs.pathExists(join(pagesDir, mode, path + '.json'))
          ).toBe(true)
        }
      }
    })
          ...
          if (mode === 'fallback-false') {
            ...
            if (!isDev) {
              ...
              await check(
                () =>
                  fs
                    .pathExists(join(pagesDir, mode, path + '.html'))
                    .then((res) => (res ? 'yes' : 'no')),
                'yes'
              )
              await check(
                () =>
                  fs
                    .pathExists(join(pagesDir, mode, path + '.json'))
                    .then((res) => (res ? 'yes' : 'no')),
                'yes'
              )
            }
